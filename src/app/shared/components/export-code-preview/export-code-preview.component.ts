...
export class ExportCodePreviewComponent implements OnInit {
  ...
  private async createCodePreview() {
    ...
    if (this.editor.configs.showComments) {
    ...
    const showGroupName = this.editor.configs.showComments && this.formatter.commentsAllowed;
  ...
  private async createCodeLines(source: StorageToken[], container: string[]) {
    for (let token of source) {
      ...
      const codeLine = this.formatter.formatToken({
        varName: this.formatter.handleVariableName(token.name, this.editor.configs.prefix),
        varValue,
      });
  ...
  private async getVariableValue(token: StorageToken) {
    const value = await this.editorSection.getStyleValue(token, this.editorSection.codePreviewConfigs);
    return this.formatter.handleVariableValue(value);
  }

  private getCodeFormatter() {
    return new this.editorSection.codeFormatters[this.editor.configs.format]();
  }
}
